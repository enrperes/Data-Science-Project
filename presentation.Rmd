...
djoko_pressure <- data %>% 
  filter(Player == "Player1" & (BP == 1 | SP == 1 | Tiebreak == 1 | (as.numeric(Player1_Score) == 40 & as.numeric(Player2_Score) == 40))) %>% 
  select(Point, Player, Outcome, Player1_Score, Player2_Score, BP, SP, Tiebreak)
dp_count <- table(djoko_pressure$Outcome)

nadal_pressure <- data %>% 
  filter(Player =="Player2" & (BP == 1 | SP == 1 | Tiebreak == 1 | (as.numeric(Player1_Score) == 40 & as.numeric(Player2_Score) == 40))) %>% 
  select(Point, Player, Outcome, Player1_Score, Player2_Score, BP, SP, Tiebreak)
```
...
Il **59°** incontro tra i due, che vede Djokovic in vantaggio per 30 a 28, durerà più di 4 ore, vedendo vincente lo spagnolo con il punteggio di **6-2 4-6 6-2 7-6(4)**. Tra i dolori al piede, le condizioni atmosferiche con umidità e freddo sfavorevoli al suo gioco carico di rotazioni, la vittoria di Nadal contro un Djokovic in grande forma è ancora più spettacolare.  
...
ggplot(djoko_pressure) +
  geom_bar(aes(x = Outcome),
            colour = Outcome) +
  labs(title = "Outcome vs. Number of Shots",
       x = "Outcome",
       y = "Number of Shots") +
  theme_minimal()+
  scale_color_manual(
